import Product, { IProduct } from "@/modules/wallapop/models/Product.model";
...
export class ProductService {
  async findAll(req: Request) {
    ...
    return products.map(p => transformProduct(p, req));
  }

  async findByIdOrFail(id: string, req: Request) { // <-- new (throws if not found)
    const product = await Product.findById(id);
    if (!product) {
      throw new Error("NOT_FOUND"); // <-- sentinel
    }
